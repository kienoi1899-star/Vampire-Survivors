...
public class SpinWeapon : Weapon
{
    ...
    void Start()
    {
        ...
        {
            fireballPrefab = fireballToSpawn;  // ✅ Lưu prefab lần đầu
        }
        else
        {
            // ✅ Nếu không assign, tự động tìm từ Holder children
            if (holder != null && holder.childCount > 0)
            {
                fireballPrefab = holder.GetChild(0);  // Lấy child đầu tiên
                Debug.LogWarning($"⚠️ SpinWeapon: fireballToSpawn không assign, dùng Holder's child: {fireballPrefab.name}");
            }
            else
            {
                Debug.LogError("❌ SpinWeapon: fireballToSpawn chưa được assign và Holder không có child!");
            }
        }
        SetStats();
    }
