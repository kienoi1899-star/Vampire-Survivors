...
public class SpinWeapon : Weapon
{
    ...
    void Update()
    {
        ...
        {
            Debug.LogWarning($"⚠️ SpinWeapon: weaponLevel {weaponLevel} out of range! Stats count: {stats.Count}");
            weaponLevel = Mathf.Clamp(weaponLevel, 0, stats.Count - 1);
        }

        if (stats[weaponLevel] == null)
        {
            Debug.LogWarning($"⚠️ SpinWeapon: stats[{weaponLevel}] is NULL!");
            return;
        }

        float currentSpeed = stats[weaponLevel].speed;
        if (currentSpeed <= 0)
        {
            Debug.LogWarning($"⚠️ SpinWeapon Level {weaponLevel}: Speed = {currentSpeed} (phải > 0!)");
        }

        holder.rotation = Quaternion.Euler(
            0f, 0f, holder.rotation.eulerAngles.z + (rotateSpeed * Time.deltaTime * currentSpeed)
        );
